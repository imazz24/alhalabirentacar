 ...
 *
 * Only the ISO code is stored per country — the flag emoji is derived from it
 * (regional indicator letters), which keeps this list short and free of
 * copy-paste mistakes.
 */
...
export const DEFAULT_COUNTRY = COUNTRIES[0]; // Lebanon

/** ISO code -> flag emoji, via the regional indicator block. */
...
/** Picks the country whose calling code a typed number starts with. */
export function countryFromNumber(value: string): Country | undefined {
  const digits = value.replace(/[^\d]/g, "");
  if (!digits) return undefined;
  // Longest dial code first, so +1268 beats +1.
  return [...COUNTRIES]
...
export function searchCountries(query: string): Country[] {
  ...
  return COUNTRIES.filter(
    (country) =>
      ...
      (digits.length > 0 && country.dial.startsWith(digits)),
  );
}
